...
ADMIN_CHAT_ID = 1402922835     # твой числовой Telegram ID
...
def t(lang, key): return DATA["texts"].get(lang, DATA["texts"]["en"]).get(key, key)
...
async def on_lang(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if photo: await q.message.reply_photo(photo, caption=welcome, parse_mode=ParseMode.HTML)
    else:     await q.message.reply_text(welcome, parse_mode=ParseMode.HTML)
...
async def on_pick_apartment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if photo_id: await q.message.reply_photo(photo=photo_id, caption=caption, parse_mode=ParseMode.HTML)
    else:        await q.message.reply_text(caption, parse_mode=ParseMode.HTML)
...
async def on_yesno(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_text(t(lang, "q_dates")); return FORM_DATES
...
async def form_dates(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(t(context.user_data.get(UD_LANG,"en"), "q_guests")); return FORM_GUESTS
...
async def form_guests(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(t(context.user_data.get(UD_LANG,"en"), "q_name")); return FORM_NAME
...
async def form_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(t(context.user_data.get(UD_LANG,"en"), "q_contact")); return FORM_CONTACT
...
async def form_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(t(context.user_data.get(UD_LANG,"en"), "q_wishes")); return FORM_WISHES
...
async def form_wishes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lang = context.user_data.get(UD_LANG, "en")
    apt = DATA["apartments"].get(context.user_data.get(UD_APT), {})
    ...
    if ADMIN_CHAT_ID:
        try: await context.bot.send_message(chat_id=ADMIN_CHAT_ID, text=admin_msg)
...
# ====== мини-Flask (для Web Service на Render) ======
...
def main():
    ...
    threading.Thread(target=run_flask, daemon=True).start()  # не мешает и в Worker
    app = build_app()
